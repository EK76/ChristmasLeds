...
the leds. The code is written in C++ programming language with the help of Arduino IDE complitator. More info about **[Arduino IDE](https://www.arduino.cc/en/software)**.

Included Arduino libraries for this project.
```
#include <DFRobotDFPlayerMini.h>   // Library used to control the DFPlayer Mini, a MP3 and WAV player module.
#include <SoftwareSerial.h>        // Library that uses standard digital pins for serial communication (UART).
```
...
### DFPlayer Mini
 ...
 or music to Arduino projects by reading files directly from a microSD card.
